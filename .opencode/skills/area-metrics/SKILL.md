---
name: area-metrics
description: 装维地市汇总查询。用于用户查看全省、山东省、各地市装维随销发展日清单、地市随销统计、生成地市随销统计报表或地市汇总，数据粒度是一行一个地市，字段包含装维发展量、全量发展量、占比、积分、人数、人均等。某个地市的装维随销发展日清单应转到 city-metrics 取该地市所有区县。
---
...
每次查询或报表流程都直接下载 `field-service-agent-area-summary` 并保存到 `temp/data/area/`。不要检查或复用 `data/area/` 历史文件，不要下载 `field-service-agent-official-staff` 或 `field-service-agent-intern-staff`。
...
- 用户说“地市随销统计”“各地市随销统计”或“各地市日清单”且没有“推送/发送/发到群/企微”时，直接下载并读取 `field-service-agent-area-summary`，在聊天框展示结果，不生成图片，不推送。
...
查询、报表生成或推送完成后，删除 `temp/data/area/`。最终 HTML/PNG 保留在 `output/`。
