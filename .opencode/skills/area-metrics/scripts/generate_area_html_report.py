...
def header_label(value) -> str:
    label = display_label(value)
    if label in COMPACT_HEADER_LABELS:
        return COMPACT_HEADER_LABELS[label]
    return escape(label)
    ...
    for col in grouped_columns:
        if "children" in col:
            ...
            for child in col["children"]:
                second_cells.append(f'<th>{header_label(child["label"])}</th>')
        else:
            top_cells.append(f'<th rowspan="2">{header_label(col["label"])}</th>')
    for col in singles:
        top_cells.append(f'<th rowspan="2">{header_label(col["label"])}</th>')
