WITH params AS (
  SELECT
    ...
    date_format(date_trunc('month', ?{_THIS_YEAR_MONTH_}), '%Y-%m-%d') AS month_start_date,
    date_format(date_parse(?{acct_day}, '%Y%m%d'), '%Y-%m-%d') AS biz_end_date,
    date_format(date_trunc('month', ?{_THIS_YEAR_MONTH_}), '%Y%m%d') AS acct_day_start,
    ?{acct_day} AS point_p_acct_day,
...
biz_city AS (
  SELECT
    ...
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.yd AS DECIMAL(18, 2)), 0) ELSE 0 END) AS yd_zw_total,
    SUM(COALESCE(TRY_CAST(s.yd AS DECIMAL(18, 2)), 0)) AS yd_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.kd AS DECIMAL(18, 2)), 0) ELSE 0 END) AS zdkd_zw_total,
    SUM(COALESCE(TRY_CAST(s.kd AS DECIMAL(18, 2)), 0)) AS zdkd_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.qwzn_fttr AS DECIMAL(18, 2)), 0) ELSE 0 END) AS fttr_h_zw_total,
    SUM(COALESCE(TRY_CAST(s.qwzn_fttr AS DECIMAL(18, 2)), 0)) AS fttr_h_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.fttrb AS DECIMAL(18, 2)), 0) ELSE 0 END) AS fttr_b_zw_total,
    SUM(COALESCE(TRY_CAST(s.fttrb AS DECIMAL(18, 2)), 0)) AS fttr_b_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.wifi_zd AS DECIMAL(18, 2)), 0) ELSE 0 END) AS wifi_zd_zw_total,
    SUM(COALESCE(TRY_CAST(s.wifi_zd AS DECIMAL(18, 2)), 0)) AS wifi_zd_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.sqznzw AS DECIMAL(18, 2)), 0) ELSE 0 END) AS sqznzw_zw_total,
    SUM(COALESCE(TRY_CAST(s.sqznzw AS DECIMAL(18, 2)), 0)) AS sqznzw_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.rhzk151 AS DECIMAL(18, 2)), 0) ELSE 0 END) AS rhzk151_zw_total,
    SUM(COALESCE(TRY_CAST(s.rhzk151 AS DECIMAL(18, 2)), 0)) AS rhzk151_total,
    SUM(CASE WHEN z.staff_id IS NOT NULL THEN COALESCE(TRY_CAST(s.tyzp AS DECIMAL(18, 2)), 0) ELSE 0 END) AS tyzp_zw_total,
    SUM(COALESCE(TRY_CAST(s.tyzp AS DECIMAL(18, 2)), 0)) AS tyzp_total
  ...
  WHERE s.p_acct_day = pms.acct_day
    ...
    AND s.new_date <= pms.biz_end_date
  GROUP BY s.area_no
),
biz_area AS (
...
point_city AS (
  ...
  LEFT JOIN zw_business_staff z
    ...
    AND z.attr_value IN ('10', '20', '50', '72', '73', '200')
  WHERE pt.acct_day >= pms.acct_day_start
